...
const VERSION = 'f99f5f0967';
const SHELL = 'pore-shell-' + VERSION;
const SHELL_FILES = ['./', './index.html', './manifest.webmanifest', './icons/app/anz.png', './icons/app/icon-192.png', './icons/app/icon-512.png', './icons/app/maskable-512.png'];
...
self.addEventListener('fetch', event => {
  ...
  // The icon sheets carry their version in the address (?v=), so a stored copy is always right: answer from the
  // cache at once, download only a version not seen yet, and drop older versions of the same sheet.
  // They live in their own cache, so a deploy that does not change the icons does not download them again.
  if (/\/icons\/sprite-(pixel|portraits)\.svg$/.test(url.pathname) && url.searchParams.has('v')) {
    event.respondWith(caches.open('pore-icons').then(cache => cache.match(req).then(hit => hit || fetch(req).then(res => {
      if (res.ok) {
  ...
  event.respondWith(
    fetch(req, { cache: 'no-cache' }).then(res => {
      if (res.ok && (url.pathname.endsWith('/') || url.pathname.endsWith('.html') || url.pathname.endsWith('.webmanifest') || /\/icons\/sprite-(pixel|portraits)\.svg$/.test(url.pathname) || url.pathname.includes('/icons/app/'))) {
